...
namespace Vn.Ntq.RoboFW.Sensors
{
    public class DistanceSensor : DeviceSensor
    {
        ...
        internal void Switch(ICollection<byte> onSensorIds)
        {
            ...
            SendCommand(new byte[] { CommandId.DISTANCE_SENSOR_TURN, (byte)sensorOnFlag });   //Send data với tham biến (0x22,sensorOnFlag)

            StartListener(CommandId.GET_DISTANCE_DATA + 0x80);  //đăng ký nhận data (0x21 + 0x80 = 0xA1)
        }
    }
}
